...
type Column struct {
	...
	IsVector  bool
	VectorDim int
	Nullable  bool // 列定义未含 NOT NULL（且非主键）
	Default   bool // 列定义含 DEFAULT 子句
	Pointer   bool // 生成时使用指针类型（nullable && !Default && !PK && !AutoInc && !Vector && !softDelete）
}
...
func parseColumn(def string, dt DDLType) (Column, error) {
	...
	c.IsVector = isVec
	c.VectorDim = dim
	if isSoftDeleteCol(c) {
		c.Pointer = false
	} else {
		c.Nullable = !strings.Contains(low, "not null") && !c.IsPK
		c.Default = strings.Contains(low, "default")
		c.Pointer = c.Nullable && !c.Default && !c.IsAutoInc && !c.IsVector
	}
	return c, nil
}
...
func dbTag(c Column) string {
	...
	// 软删除字段：加 ,logic 使其「自包含」生效，不依赖 DB 级 SoftDeleteField 配置。
	if isSoftDeleteCol(c) {
...
// softDeleteNames 是约定软删除字段名（列名或 Go 字段名）。类型须为 time/bool/int 才启用。
var softDeleteNames = map[string]bool{
	"deleted_at": true, "deleted": true, "is_deleted": true, "is_del": true,
	"del_at": true, "delete_at": true, "deletedat": true, "isdeleted": true,
}

func isSoftDeleteCol(c Column) bool {
	if !softDeleteNames[strings.ToLower(c.ColName)] && !softDeleteNames[strings.ToLower(c.GoName)] {
		return false
	}
	switch c.GoType {
	case "time.Time", "bool",
...
// goTypeDecl 返回列在生成结构体时应使用的 Go 类型声明：可空无默认列用指针类型。
func goTypeDecl(c Column) string {
	if c.Pointer {
		return "*" + c.GoType
	}
	return c.GoType
...
func generateModel(t Table, pkg, tableName string) (string, error) {
	...
	for _, c := range t.Columns {
		if isSoftDeleteCol(c) {
			fmt.Fprintf(buf, "\t// %s 软删除字段（框架自动识别，Delete 走逻辑删除；物理删除用 ForceDelete）\n", c.GoName)
		}
		fmt.Fprintf(buf, "\t%s %s %s\n", c.GoName, goTypeDecl(c), fmt.Sprintf("`db:%q`", dbTag(c)))
	}
	fmt.Fprintf(buf, "}\n\n")
...
// appendExtrasORM 在 files 中追加 example.go（使用示例）与可选 <struct>_repo.go（Repo 脚手架）。
func appendExtrasORM(files map[string]string, tables []Table, pkg string, opts Options) error {
	if opts.Example {
		ex, err := generateExampleORM(tables[0], pkg)
		if err != nil {
			return err
		}
		files["example.go"] = ex
...
// insertLit 返回字段在示例插入语句里的字面量。
func insertLit(c Column) string {
	var lit string
	switch {
	case c.IsVector:
		lit = "[]float32{ /* 维度需与表 vector(N) 一致，通常来自 embedding 模型 */ }"
	case c.GoType == "string":
		lit = `"example"`
	case c.GoType == "bool":
		lit = "false"
	case strings.HasPrefix(c.GoType, "int"), strings.HasPrefix(c.GoType, "uint"):
		lit = "1"
	case strings.HasPrefix(c.GoType, "float"):
		lit = "1.0"
	case c.GoType == "time.Time":
		lit = "time.Now()"
	case c.GoType == "[]byte":
		lit = `[]byte("x")`
	case c.GoType == "map[string]any":
		lit = "map[string]any{}"
	case c.GoType == "any":
		lit = "nil"
	default:
...
// queryLit 返回查询示例里可用的字面量与是否可用（复杂类型返回不可用）。
func queryLit(c Column) (string, bool) {
	var lit string
	switch {
	case c.IsVector:
		return "", false
...
// firstSimpleField 选一个适合做查询示例的标量字段（非向量、有可用字面量）。
func firstSimpleField(t Table) (Column, bool) {
	for _, c := range t.Columns {
		if _, ok := queryLit(c); ok {
			return c, true
		}
	}
	return Column{}, false
...
// generateExampleORM 生成「生成物即所用」示例文件：插入 / 查询 / 更新删除 / Repo / 向量近邻。
func generateExampleORM(t Table, pkg string) (string, error) {
	hasTime := false
	for _, c := range t.Columns {
		if c.GoType == "time.Time" {
			hasTime = true
	...
	cols := S + "Cols"
	var b bytes.Buffer
	b.WriteString("// Code generated by gobreath-orm/cmd/ormgen. DO NOT EDIT.\n")
	b.WriteString("// 本文件为「生成物即所用」示例代码：演示如何用生成的模型与列闭包完成常见操作。\n")
	...
	b.WriteString("// 把下面的 exampleDB 替换成你自己的 *orm.DB（如 orm.Open(\"postgres\", dsn)）。\n")
	b.WriteString("// 此处仅作示例占位，避免为生成代码引入具体数据库驱动依赖。\n")
	b.WriteString("var exampleDB *orm.DB\n\n")

	// 1) 插入
	b.WriteString("// 1) 插入（零字段名：值通过结构体字段传入，列名由 " + cols + " 闭包管理）。\n")
	b.WriteString("func ExampleInsert(ctx context.Context) error {\n")
	b.WriteString("\te := " + S + "{\n")
	for _, c := range t.Columns {
		if c.IsAutoInc || isSoftDeleteCol(c) {
			continue // 自增 / 软删除字段留零值
		}
		b.WriteString("\t\t" + c.GoName + ": " + insertLit(c) + ",\n")
	}
	b.WriteString("\t}\n")
	b.WriteString("\treturn orm.Insert[" + S + "](ctx, exampleDB, &e)\n")
	b.WriteString("}\n\n")

	fld, ok := firstSimpleField(t)
	if ok {
		lit, _ := queryLit(fld)
		// 2) 查询
		b.WriteString("// 2) 查询（用生成的列闭包代替手写字段名，重构不怕拼错）。\n")
		b.WriteString("func ExampleQuery(ctx context.Context) ([]" + S + ", error) {\n")
		b.WriteString("\tq := orm.NewQuery[" + S + "]().\n")
		b.WriteString("\t\tEq(" + cols + "." + fld.GoName + ", " + lit + ").\n")
		b.WriteString("\t\tOrderBy(" + cols + "." + fld.GoName + ", false)\n")
		b.WriteString("\treturn orm.SelectList[" + S + "](ctx, exampleDB, q)\n")
		b.WriteString("}\n\n")
		// 3) 更新 / 删除
		b.WriteString("// 3) 更新 / 删除（软删除字段自动走逻辑删除，物理删除用 repo.ForceDelete）。\n")
		b.WriteString("func ExampleUpdateDelete(ctx context.Context) error {\n")
		b.WriteString("\tq := orm.NewQuery[" + S + "]().Eq(" + cols + "." + fld.GoName + ", " + lit + ")\n")
		b.WriteString("\tif err := orm.Update[" + S + "](ctx, exampleDB, q, &" + S + "{ /* 要更新的字段 */ }); err != nil {\n\t\treturn err\n\t}\n")
		b.WriteString("\treturn orm.Delete[" + S + "](ctx, exampleDB, q)\n")
		b.WriteString("}\n\n")
		// 4) Repo
		b.WriteString("// 4) Repo[T] 仓储（推荐：免类型参数，DAO 风格）。\n")
		b.WriteString("func ExampleRepo(ctx context.Context) ([]" + S + ", error) {\n")
		b.WriteString("\trepo := orm.NewRepo[" + S + "](exampleDB)\n")
		b.WriteString("\tq := orm.NewQuery[" + S + "]().Eq(" + cols + "." + fld.GoName + ", " + lit + ")\n")
		b.WriteString("\treturn repo.SelectList(ctx, q)\n")
		b.WriteString("}\n\n")
	}
	...
	for _, c := range t.Columns {
		if c.IsVector {
			b.WriteString("// 5) 向量近邻检索（AI / RAG 场景；表含向量列 " + c.GoName + "）。\n")
			b.WriteString("func ExampleVectorSearch(ctx context.Context) ([]" + S + ", error) {\n")
			b.WriteString("\tvec := []float32{ /* 与 " + c.GoName + " 同维度的向量，通常来自 embedding 模型（如千问 text-embedding-v3） */ }\n")
			b.WriteString("\tq := orm.NewQuery[" + S + "]().\n")
			b.WriteString("\t\tNearestBy(" + cols + "." + c.GoName + ", vec, 5, orm.Cosine)\n")
			b.WriteString("\treturn orm.SelectList[" + S + "](ctx, exampleDB, q)\n")
			b.WriteString("}\n\n")
			break
		}
...
// generateRepoORM 生成 Repo[T] 便捷构造脚手架（等价于 orm.NewRepo[T](db)）。
func generateRepoORM(t Table, pkg string) (string, error) {
	S := t.StructName
	var b bytes.Buffer
	b.WriteString("// Code generated by gobreath-orm/cmd/ormgen. DO NOT EDIT.\n\n")
	b.WriteString("package " + pkg + "\n\n")
	b.WriteString("import orm \"github.com/wusenshan/gobreath-orm\"\n\n")
	b.WriteString("// New" + S + "Repo 创建 " + S + " 的泛型仓储句柄（便捷构造，等价于 orm.NewRepo[" + S + "](db)）。\n")
	b.WriteString("func New" + S + "Repo(db *orm.DB) *orm.Repo[" + S + "] {\n")
	b.WriteString("\treturn orm.NewRepo[" + S + "](db)\n")
	b.WriteString("}\n")
	return formatBuf(&b)
}
