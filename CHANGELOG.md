...
- **fix: `Query` 被调用方复用时会带上上一次查询的上下文（无软删除列的模型上 `SelectOne` 静默只取 1 行）**：`applyLogic` 在「模型没有生效的逻辑删除列」或「已显式 `Unscoped`」时走短路径，**把调用方的 `Query` 原样返回**（另一分支则返回副本）；而它的调用方紧接着链的是**就地修改**的 `WithDialect(db.dialect)` / `WithPrefix(db.prefix)` / `Limit(1)`。于是这些写入落到了调用方的 `q` 上，出现三种症状，全都不报错：(1) `SelectOne` 内部补的 `LIMIT 1` 钉在调用方，之后用同一个 `q` 查列表**只拿回 1 行**（真库实测 3 行变 1 行）；(2) `db` 的方言与表前缀写进 `q`，之后 `q.ToSQL()`（文档承诺「不会补上 db 级表前缀、方言」）凭空变成 `` SELECT * FROM `t_users` ... WHERE ... LIKE ? ``，`$1` 也变成了 `?`；(3) `Page` 另有一处同源问题 —— 直接 `q.Limit(size).Offset(...)` 把分页参数写回调用方。**只有无软删除列的模型会中招**，带 `,logic` 的模型因为本来就返回副本而免疫 —— 这也是既有用例（大多用带软删除的 `User`）全都看不见它的原因。现 `applyLogic` 无论是否追加条件**一律返回副本**，`Page` 也在内部副本上设 `limit/offset`。新增 `TestApplyLogicAlwaysReturnsFreshQuery`（含两条短路径）、`TestSelectOneDoesNotPinLimitOnCallerQuery`、`TestQueryEntrypointsDoNotStampDBContextOnCallerQuery`（7 个入口 × 断言「入口前后 `ToSQL()` 逐字不变」，且先断言内部 SQL 确实带了前缀以免空跑）与真库版 `TestQueryReuseAcrossEntrypoints`（4 后端 × 5 入口，断言复用后仍能取到全部 3 行）。**mutation check**：把 `applyLogic` 短路径改回 `return q` 后，真库用例 4 后端 × 5 子用例**全部失败**，报文正是 `"pref_users"` → `` `t_pref_users` ``、`$1` → `?`、末尾多出 `LIMIT 1`（`query.go` + `crud.go` + `query_reuse_test.go` + `integration/query_reuse_it_test.go`）。
